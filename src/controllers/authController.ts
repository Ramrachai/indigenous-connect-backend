...
// Some disposable domain for testing
// mailinator.com
// 10minutemail.com
// guerrillamail.com
// @route /auth/checkdisposabledemail/:email
export const checkDisposableEmail = async (req: Request, res: Response) => {
  const validatedData = emailSchema.safeParse({ email: req.params.email });

  if (!validatedData.success) {
    return res.status(400).json({ message: validatedData.error.errors[0].message });
  }

  const url = `https://mailcheck.p.rapidapi.com/?domain=${validatedData.data.email}`;
  const config = {
    method: "GET",
    headers: {
      "x-rapidapi-key": process.env.CHECK_EMAIL_RAPID_API_KEY as string,
  ...
  try {
    ...
    res.status(200).json({
      disposable: false,
      message: "Email is valid and not disposable.",
      result,
    });
  } catch (error) {
    console.log("Error in checkDisposableEmail", error);
